...
#| label: load-data
migration <- tryCatch(
  read.csv("../../../files/data/external_data/migration.csv"),
  error = function(e) read.csv("files/data/external_data/migration.csv")
) |> 
    as_tibble()
...
If you're working on the website, the code above reads from the course site's local `files/` directory. If you're working from the project root in RStudio or Positron, the fallback path will use `files/data/external_data/migration.csv`. The `read.csv()` function is used here due to bugs related to the Web-R application.
::: 
