...
build_schedule_for_page <- function(schedule_file) {
  schedule <- read_csv(schedule_file, show_col_types = FALSE) %>%
    ...
    mutate(
      date = mdy(date),
      deadline_actual = update(date, hour = hour(deadline), minute = minute(deadline)),
    ...
    mutate(
      var_lesson = ifelse(
        !is.na(lesson),
        glue('<a href="{lesson}.qmd"><i class="fa-solid fa-book-open-reader fa-lg"></i></a>'),
    ...
    mutate(
      ...
      mutate(
        var_lab = ifelse(
          !is.na(lab),
          glue('<a href="{lab}.qmd"><i class="fa-solid fa-chalkboard-teacher fa-lg"></i></a>'),
          glue('<font color="#e9ecef"><i class="fa-solid fa-chalkboard-teacher fa-lg"></i></font>')
        )
      ) %>%
...
build_ical <- function(schedule_file, base_url, page_suffix, class_number) {
  ...
  schedule <- read_csv(schedule_file, show_col_types = FALSE) %>%
    mutate(session = if_else(is.na(title), glue(""), glue("({subgroup}) "))) %>%
    ...
    mutate(url = coalesce(title, lesson),
